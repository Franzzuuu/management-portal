...
export default function SearchableVehicleSelect({ value, onChange, className = '' }) {
                ...
                try {
                    ...
                    if (isMounted) {
                        if (data.success && Array.isArray(data.vehicles)) {
                            // Filter for only approved vehicles without RFID tags
                            const untaggedVehicles = data.vehicles.filter(v =>
                                v.approval_status === 'approved' && v.rfid_status === 'untagged'
                            );
